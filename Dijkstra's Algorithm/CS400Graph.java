...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.dataSequence = new LinkedList<>(); //new dataSequence that will be deep copied too

            for(int i = 0; i < copyPath.dataSequence.size(); i++) { //deep copies copyPath dataSequence to new path objects dataSequence
                Vertex currVertex = new Vertex(copyPath.dataSequence.get(i)); //create a new vertex to be added to dataSequence
                this.dataSequence.add(currVertex.data);
            }

            this.dataSequence.add(extendBy.target.data);
            this.distance += extendBy.weight; //adds the weight of extendBy to the distance
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if(!this.vertices.containsKey(start) || !this.vertices.containsKey(end)) { //checks if vertices provided are actually in vertices hashtable
            throw new NoSuchElementException("One of the vertices provided is not in this graph, please try again");
        }

        PriorityQueue<Path> frontierPath = new PriorityQueue<>();//frontier
        Path startPath = new Path(vertices.get(start));
        frontierPath.add(startPath);//adds start variable path to the frontier to start while loop
        ArrayList<Path> discoveredPaths = new ArrayList<>();//keeps track of discovered paths

        while(!frontierPath.isEmpty()) { //while the frontier still has paths to be checked, loop continues
            Path currPath = frontierPath.poll();//picks the path at the top of the queue and removes it
            if(currPath.end.data.equals(end)) { //checks if current path end variable is same as end variable provided
                return currPath; //if so it will return the current path as it is the shortest path
            }


            for(int k = 0; k < currPath.end.edgesLeaving.size(); k++) {
                Path addPath = new Path(currPath, currPath.end.edgesLeaving.get(k));
                if(!discoveredPaths.contains(addPath)) {
                    frontierPath.add(addPath);//adds path to frontier
                    discoveredPaths.add(addPath);//add path to discoveredPath so we don't add again
                }
            }
        }
        //if while loop terminates, then frontier is empty and no path has been found, throw a NoSuchElementException
        throw new NoSuchElementException("No path found between vertices provided, please try again");
    }
